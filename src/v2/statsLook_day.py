...
class RatioDailyAnalyzer:
    """Encapsulates the entire BTC/ETH daily ratio analysis (stats + bootstrap + interactive range + chart)."""

    def __init__(self, csv_path: str = 'btc_eth_day_paired.csv'):
    ...
    def _load_and_clean_data(self):
        ...
        print(f"✅ Loaded {len(self.df):,} daily observations")
    ...
    def overall_statistics(self):
        ...
        print(f"Positive days      : {(self.changes > 0).mean()*100:.2f}%  ({(self.changes > 0).sum()}/{len(self.changes)})")
    ...
    def block_bootstrap_overall_mean(self):
        ...
        print(f"Using {len(self.changes_np):,} daily observations | Block size = 21 days (~3 weeks)")
    ...
    def setup_dynamic_balanced_range(self):
        ...
        print(f"   Covers {coverage_pct:.1f}% of all {len(self.changes)} historical days")
    ...
    def generate_and_save_visuals(self):
        ...
        plt.xlabel(f'Bootstrapped {self.PERCENTILE}th Percentile of |Daily Change| (%)')
        ...
        plt.figtext(0.5, 0.015,
                    ...
                    f"±{self.balanced_range_pct}% around the 10:00 KST ratio • Covers {self.coverage_pct:.1f}% of {len(self.changes)} historical days\n"
        ...
        png_filename = f"ratio_daily_analysis_{int(self.PERCENTILE)}.png"
...
if __name__ == "__main__":
    analyzer = RatioDailyAnalyzer()   # you can also pass a custom path: RatioDailyAnalyzer('my_data.csv')
    analyzer.run()
